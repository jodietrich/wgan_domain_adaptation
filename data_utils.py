...
class DataSampler(object):
    def __init__(self, train_images, validation_images):
        self.shape = list(exp_config.image_size).append(exp_config.n_channels) # [x, y, z, #channels]
...
# batch is sequential images. Ask Christian if reshuffling is needed.
    def __call__(self, batch_size):
        next_index = (self.current_index + batch_size) % self.train_data.shape[0]
        batch = self.train_data[self.current_index:next_index]
        self.current_index = next_index
        return self.data2img(batch)

    def get_validation_batch(self, batch_size):
        next_index = (self.current_index + batch_size) % self.validation_data.shape[0]
