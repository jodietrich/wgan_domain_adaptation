...
with sess1.as_default():
    thresh_out = tf_utils.put_kernels_on_grid(make_test_tensor(thresh_test), rescale_mode='centered', input_range=(0,2), cutoff_abs=1.0).eval()
    scale1_out = tf_utils.put_kernels_on_grid(make_test_tensor(scale_test1), rescale_mode='centered', input_range=(0,2)).eval()
    scale2_out = tf_utils.put_kernels_on_grid(make_test_tensor(scale_test2), rescale_mode='centered', input_range=(0,2)).eval()
...
with sess2.as_default():
    ...
    print('CENTERED RESCALING')
    cimage = tf_utils.put_kernels_on_grid3d(images_tensor, 2, 50, rescale_mode='centered').eval()
    # print(np.squeeze(autoimage))
    plt.figure()
    plt.imshow(np.squeeze(cimage), cmap='gray')
